...
<AccordionGroup>
  ...
  <Accordion icon="shield" title="safeGet - 安全读取字段">
  ...
  <Accordion icon="shield" title="safeGetOr - 安全读取字段并提供默认值">
    在 `safeGet` 的基础上提供默认值；当取值失败时返回默认值。
    ```yaml
...
<AccordionGroup>
  ...
  <Accordion icon="play" title="性能">
    - 避免在模板中进行复杂的计算
    - 合理使用缓存机制
    - 对于大量数据处理，考虑在上游服务中预处理
  </Accordion>

  <Accordion icon="gear" title="维护性">
    - 保持模板简洁明了
    - 添加适当的注释说明
    - 使用有意义的变量名
  </Accordion>
